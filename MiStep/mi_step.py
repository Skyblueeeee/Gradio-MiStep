...
class MiFit():
    def __init__(self):
        pass
    #获取北京时间确定随机步数&启动主函数
    def getBeijinTime(self,user,step_min,step_max):
        try:
            result = self.main(user,int(step_min),int(step_max))
            return result
    ...
    # 登录
    def login_info(self,_user,_passwd):
        user = str(_user)
        ...
        self.login_token = 0
        self.login_token, self.userid = self.login(user, password)
        if self.login_token == 0:
            print("登陆失败！")
            return "Login Fail!"
        else:
            print("登录成功")
    ...
    def main(self,user,min_1, max_1):
        if 1:
            step = ''
            if step == '':
            ...
            app_token = self.get_app_token(self.login_token)
...
if __name__ == "__main__":
    ...
    mifit = MiFit()
    print(mifit.login_info(USER,PASSWD))
    print(mifit.getBeijinTime(USER,STEP_MIN,STEP_MAX))